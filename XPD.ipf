...
Function/DF CreateDAQPackageData() // Called only from GetDAQPackageDFREF
	...
	Variable/G dfraq:AEPPcked = 1 
	Variable/G dfraq:CstSldAngCked = 0 
...
Function StartAcqBtnProc(ctrlName) : ButtonControl
	...
	NVAR/SDFR=dfraq Is1st, ClvNum, ChNum, RawCked, MaxCked, WholeCked, Com1Cked, Com2Cked, Com3Cked, Com4Cked, CkNum
    ...
    NVAR/SDFR=dfraq PkSel1, PkSel2, PkSel3, PkSel4

    Variable/G dfrfit:PeakNum1 = 0
    Variable/G dfrfit:PeakNum2 = 0
    Variable/G dfrfit:PeakNum3 = 0
    Variable/G dfrfit:PeakNum4 = 0
    Variable/G dfrfit:Is1stFit1 = 0
    Variable/G dfrfit:Is1stFit2 = 0
    Variable/G dfrfit:Is1stFit3 = 0
    Variable/G dfrfit:Is1stFit4 = 0
    Variable/G dfrfit:IsFitting1 = 0
    Variable/G dfrfit:IsFitting2 = 0
    Variable/G dfrfit:IsFitting3 = 0
    Variable/G dfrfit:IsFitting4 = 0
    Variable/G dfrfit:FitErrCnt = 0

    NVAR/SDFR=dfrfit PeakNum1, PeakNum2, PeakNum3, PeakNum4, Is1stFit1, Is1stFit2, Is1stFit3, Is1stFit4, IsFitting1, IsFitting2, IsFitting3, IsFitting4, FitErrCnt

    Make/O/N=2 dfraq:ColorSclValW = {0, 100}
    Make/O/N=2/T dfraq:ColorSclNameW = {"Min", "Max"}
...
Function ReadAndDisplay(s)
	...
	Wave/SDFR=dfraq DispW, ColorSclValW, ColorSclNameW
	...
	do
		...
		elseif(LNum > 2) //there are data in the file
			...
			if(cmpstr(FsTrans, "OK") != 0)
				if(LNum == 3 && PreLnum != Lnum) //avoid line 3 has been selected to do 2 componets fitting.
					FsTrans = trans
					ClvCnt += 1
					PreLnum = Lnum
			...
			if(StgPosNum == 0 && nth == 3 && com3cked == 1 && PeakNum3 == 0) // check if com3cked = 1
				PkSel3 = 0
			...
			if(StgPosNum == 0 && nth == 4 && com4cked == 1 && PeakNum4 == 0) // check if com4cked = 1
				PkSel4 = 0
			...
			if(ridx == 0) //only at the first sprectrum, StgPosNum == 0 && nth == 1
				...
				if(V_flag == 0)
					...
					ColorScale/W=XPD_Color_Scale/C/N=XPDColorScale/F=0/A=MC/D=2/E=2/X=0.00/Y=0.00 fstyle=1,tickThick=2.00,ctab={0,100,YellowHot,0},widthPct=100,vert=0,trace={XPDWin1,polarY0},userTicks={ColorSclValW,ColorSclNameW}
				endif
			endif	
...
Function DispAzimProf()
	...
	if (V_Flag == 0) // if the Azimuthal Profile window doesn't exists
		...
		ModifyGraph tick(left)=2,tick(bottom)=2,mirror=1,fSize(bottom)=12,fSize(left)=12;DelayUpdate
		ModifyGraph font(bottom)="Arial",font(left)="Arial";DelayUpdate
		Label left "\\F'Arial'\\Z13Anisotropy (a.u.)";DelayUpdate
		Label bottom "\\F'Arial'\\Z13Azimuthal Angle (deg.)";DelayUpdate
		ModifyGraph lblPosMode(left)=4,lblPos(left)=35, lblPosMode(bottom)=4,lblPos(bottom)=30
	endif
End
...
Function DispRadProf()
	...
	if(MinPhi<5 && MaxPhi>355)
		Duplicate/O/Free dfrp:RdsW_BfExt, NotExtThtW
		Duplicate/O/Free dfrp:PhiW_BfExt, NotExtPhiW
		Duplicate/O/Free dfrp:ItsW_BfExt, NotExtItsW
	else
	...
	if (V_Flag == 0) // if the Azimuthal Profile window doesn't exists
		...
		Label left "\\F'Arial'\\Z13Anisotropy (a.u.)";DelayUpdate
		Label bottom "\\F'Arial'\\Z13Azimuthal Angle (deg.)";DelayUpdate
		ModifyGraph lblPosMode(left)=4,lblPos(left)=30, lblPosMode(bottom)=4,lblPos(bottom)=30
	endif
End
	...
	elseif(tht == CrnTht && phi != CrnPhi) //when the sample moves to next azimuthal position
		...
		if(aznum > 1)
			...
			for(a = StgPosNum+1-aznum; a < StgPosNum; a += 1)
				iw[a] = (itswr[a] - azmean)/azmean
			endfor
			its = (its - azmean)/azmean
		endif
...
Function InitGenStaSerPanel()
	...
	if(V_Flag == 0) //if no GenStaSerPanel
		NewPanel/N=GenStaSerPanel/k=1/W=(800,50,1130,375) as "Generate Angle Scan File"
        CheckBox AEPPck Win=GenStaSerPanel, pos={20, 20}, size={250,20},title="Azimuthal equidistant polar projection method", font="Arial", fsize=12, mode=1, value=1, proc=AEPPckProc, help={"Check to generate an angle scan file for AEPP method."};DelayUpdate
		CheckBox CstSldAngCk Win=GenStaSerPanel, pos={20, 40}, size={50, 20}, title="Constant solid-angle density mode", font="Arial", fsize=12, mode=1, value=0, proc=CstSldAngCkProc, help={"Check to generate an angle scan file for constant solid-angle density."};DelayUpdate
//        
		TitleBox PolarTxt win=GenStaSerPanel, pos={20, 60}, size={150,20}, title="Polar angles (deg.):",font="Arial", fsize=12, frame=0;DelayUpdate
		SetVariable PlStart win=GenStaSerPanel, pos={30, 80}, size={70,20}, title="Start:";DelayUpdate
		SetVariable PlStart win=GenStaSerPanel, font="Arial", fsize=12, value=dfraq:plstart, limits={0,90,1};DelayUpdate
		SetVariable PlStop win=GenStaSerPanel, pos={110, 80}, size={70,20}, title="Stop:";DelayUpdate
		SetVariable PlStop win=GenStaSerPanel, font="Arial", fsize=12, value=dfraq:plstop, limits={0,90,1};DelayUpdate
		SetVariable PlStep win=GenStaSerPanel, pos={190, 80}, size={70,20}, title="Step:";DelayUpdate
		SetVariable PlStep win=GenStaSerPanel, font="Arial", fsize=12, value=dfraq:plstep, limits={0,90,0.1};DelayUpdate
		
		TitleBox AzimTxt win=GenStaSerPanel, pos={20, 105}, size={150,20}, title="Azimuthal angles (deg.):",font="Arial", fsize=12, frame=0;DelayUpdate
		SetVariable azStart win=GenStaSerPanel, pos={30, 125}, size={70,20}, title="Start:";DelayUpdate
		SetVariable azStart win=GenStaSerPanel, font="Arial", fsize=12, value=dfraq:azstart, limits={0,90,1};DelayUpdate
		SetVariable azStop win=GenStaSerPanel, pos={110, 125}, size={70,20}, title="Stop:";DelayUpdate
		SetVariable azStop win=GenStaSerPanel, font="Arial", fsize=12, value=dfraq:azstop, limits={0,180,1};DelayUpdate
		SetVariable azStep win=GenStaSerPanel, pos={190, 125}, size={100,20}, title="Initial step:";DelayUpdate
		...
		SetVariable PosNumVal win=GenStaSerPanel, pos={20, 155}, size={200,20}, title="The number of the positons:";DelayUpdate
		SetVariable PosNumVal win=GenStaSerPanel, font="Arial", fsize=12, value=dfraq:PosNum, limits={0,0,0};DelayUpdate
		Button CalNumBtn win=GenStaSerPanel, pos={225, 155}, size={60,20}, title="Calculate";DelayUpdate
		Button CalNumBtn win=GenStaSerPanel, font="Arial", fsize=12, disable=0, proc=CalNumBtnProc;DelayUpdate
	
		Button GenStaSerBtn win=GenStaSerPanel, pos={125, 190}, size={60,20}, title="Generate";DelayUpdate
		Button GenStaSerBtn win=GenStaSerPanel, font="Arial", fsize=12, disable=0, proc=GenStaSerBtnProc;DelayUpdate
		
		TitleBox NoteTxt win=GenStaSerPanel, pos={20, 215}, size={150,20}, title="Note:",font="Arial", fsize=12, frame=0;DelayUpdate
		...
		TitleBox Note1Txt win=GenStaSerPanel, pos={20, 230}, size={150,20}, title="1. Only 1 point when Polar = 0 (normal emission). ",font="Arial", fsize=12, frame=0;DelayUpdate
		TitleBox Note2Txt win=GenStaSerPanel, pos={20, 245}, size={150,20}, title="2. For azimuthal equidistant polar projection method, ",font="Arial", fsize=12, frame=0;DelayUpdate
		TitleBox Note2Txt2 win=GenStaSerPanel, pos={25, 260}, size={150,20}, title="initial step is the azimuthal step at largest polar angle.",font="Arial", fsize=12, frame=0;DelayUpdate
		TitleBox Note3Txt win=GenStaSerPanel, pos={20, 275}, size={150,20}, title="3. For constant solid-angle density mode,",font="Arial", fsize=12, frame=0
		TitleBox Note3Txt2 win=GenStaSerPanel, pos={25, 290}, size={150,20}, title="azimuthal steps are determined by polar steps. ",font="Arial", fsize=12, frame=0
	endif
End
...
Function AEPPckProc(ctrlName, Cked) : CheckBoxControl
	String ctrlName
	Variable Cked
	DFREF dfraq = GetDAQPackageDFREF()
	NVAR/SDFR=dfraq AEPPcked, CstSldAngCked

	if(Cked)
		AEPPcked = 1
		CstSldAngCked = 0
		CheckBox AEPPck value = 1
		CheckBox CstSldAngCk value = 0
        SetVariable azStep win=GenStaSerPanel, disable=0
	else
        AEPPcked = 0
		CstSldAngCked = 1
		CheckBox AEPPck value = 0
		CheckBox CstSldAngCk value = 1
        SetVariable azStep win=GenStaSerPanel, disable=1
	endif
End

Function CstSldAngCkProc(ctrlName, Cked) : CheckBoxControl
	String ctrlName
	Variable Cked
	DFREF dfraq = GetDAQPackageDFREF()
	NVAR/SDFR=dfraq AEPPcked, CstSldAngCked
...
Function GenStaSerBtnProc(ctrlName): ButtonControl
	String ctrlName
	DFREF dfraq = GetDAQPackageDFREF()
	NVAR/SDFR=dfraq plstart, plstop, plstep, azstart, azstop, azstep0, azbl, AEPPcked
...
Function CalNumBtnProc(ctrlName): ButtonControl
	String ctrlName
	DFREF dfraq = GetDAQPackageDFREF()
	NVAR/SDFR=dfraq plstart, plstop, plstep, azstart, azstop, azstep0, azbl, PosNum, AEPPcked
