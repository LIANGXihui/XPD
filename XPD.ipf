...
Function StartAcqBtnProc(ctrlName) : ButtonControl
	...
	if(Is1st == 1)
		...
		if(V_flag != 0)
			...
			Abort "I cannot find the Dispersion Factors! \nPlease import it manully by clicking XPD menu -> Import dispersion factors. \n\nStop acqusition."
		elseif(V_flag == 0)
			SetDataFolder dfraq
...
Function ProcessTab(show)
	...
	SetVariable CropStPhiBox,Win=Processpanel, help={"Input the starting Phi angle to crop the pattern, greater than which the patten within the range will be remained and the other parts will be cut off."}
	...
	SetVariable CropStPolarBox,Win=Processpanel, help={"Input the starting Polar angle to crop the pattern, greater than which the patten within the range will be remained and the other parts will be cut off."}
	...
	GroupBox SmImgGrBox,Win=Processpanel, font="Arial",fStyle=1, help={"Smooth the pattern by interpolation. The smoothed pattern will be displayed as an image whose resolution can be set by users."}
	SetVariable RslX,Win=Processpanel, pos={20,385},size={113,18},title="Resolution:", disable=state, help={"Input the image resolution of the smoothed pattern."}
	SetVariable RslX,Win=Processpanel, font="Arial",limits={10,1000,10},value= dfrp:Xrsl
	SetVariable RslY,Win=Processpanel, pos={136,385},size={56,18},title="X", disable=state
	SetVariable RslY,Win=Processpanel, font="Arial",limits={10,1000,10},value= dfrp:Yrsl
...
Function/DF CreateXPDPackageData() // Called only from GetPackageDFREF
	...
	Variable/G dfrp:Xrsl = 100 // X resolution
	Variable/G dfrp:Yrsl = 100 // Y resolution
...
Function DispRadProf()
	DFREF dfr = GetXPDPackageDFREF()
	DFREF dfrp = dfr:Process
	Wave/SDFR=dfrp RdsW, PhiW, ItsW, ThtW, PhiW_BfExt, RdsW_BfExt, ItsW_BfExt 
	NVAR/SDFR=dfrp SelPhiAng, SelPhiWid
...
Function PolarCropBtnProc(ctrlName) : ButtonControl
	...
	delval2=WaveMax(RdsW)-(Polar0+IncAng)
...
Function DoSmImgBtnProc(ctrlName) : ButtonControl
	...
	DFREF dfrp = dfr:Process
	Wave/SDFR=dfrp XPDM, RdsW, PhiW, ItsW
	NVAR/SDFR=dfrp Xrsl, Yrsl, UseStereo
	...
	if(UseStereo == 1)
		...
		sprintf cmdstr, "XYZtoMatrix(\"xwct\",\"SmthImgStgPrj\",\"ywct\",%u,\"ItsW\",%u,2,1)", Xrsl, Yrsl 
		Execute cmdstr
		...
		xwct = -rdsw[p]*cos(phiw[p]*pi/180)
		ywct = rdsw[p]*sin(phiw[p]*pi/180)
		sprintf cmdstr2, "XYZtoMatrix(\"xwct\",\"SmthImgEqPrj\",\"ywct\",%u,\"ItsW\",%u,2,1)", Xrsl, Yrsl 
		Execute cmdstr2
...
Function HookCsrToPickPixel(s)
	...
	DFREF dfr = GetXPDPackageDFREF()
	DFREF dfrp = dfr:Process
	DFREF dfrwm = root:WMPolarGraphs:ProcessGraphWin
